...
```
最后，我们介绍两个非常实用的工具函数 **triu**（上三角）与 **tril**（下三角），它们在计算 **因果注意力掩码（causal attention mask）** 时非常有用。在语言模型中，为了确保模型在预测第 t+1 个词时只能看到第 t 个词及其之前的词（含自身，即不能偷看未来信息），就需要用到因果掩码。

按照主流实现的约定（GPT-2、LLaMA 等都是如此），注意力矩阵 A[i, j] 表示第 i 个位置（query，对应行）对第 j 个位置（key，对应列）的注意力权重。因果性要求 query i 只能关注 j ≤ i 的位置，即 A[i, j] 在 j > i 时为 0，因此非零元素恰好构成 **下三角矩阵（含对角线）**：

```
# 实用操作：下三角矩阵（用于因果注意力掩码）
x = torch.ones(3, 3).tril()  # 创建一个3x3全1矩阵，然后取其下三角部分（含对角线）
# 结果：
# [[1, 0, 0],
#  [1, 1, 0],
#  [1, 1, 1]]
# 含义：第0个词只能看到第0个词，第1个词能看到第0、1个词，依此类推
```

对应的 **triu** 则保留上三角部分（同样含对角线）。它与因果掩码的关系有两种常见视角：一是如果把矩阵约定反过来，即 M[i, j] 表示位置 i 对位置 j 的贡献（i 为源/key、j 为目标/query），那么同样的因果掩码就呈上三角，与上面的下三角互为转置；二是很多代码库习惯先用 triu 构造严格上三角再取反，等价地得到下三角掩码：

```
x = torch.ones(3, 3).triu()   # 上三角（含对角线）
# [[1, 1, 1],
#  [0, 1, 1],
#  [0, 0, 1]]

y = torch.ones(3, 3).triu(1)  # 严格上三角（不含对角线），取反后即为因果掩码
# [[0, 1, 1],
#  [0, 0, 1],
#  [0, 0, 0]]
```
